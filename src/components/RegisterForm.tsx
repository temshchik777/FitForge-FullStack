...
export function RegisterForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateName(formData.firstName))
      next.firstName = "Ім'я має містити лише літери";
    if (!validateName(formData.lastName))
      next.lastName = "Прізвище має містити лише літери";
    if (!validateLogin(formData.login))
      next.login = "Логін: латиниця і цифри (3–20)";
    if (!validateEmail(formData.email)) next.email = "Некоректний email";
    if (!validatePassword(formData.password, { min: 7, max: 30 }))
      next.password = "Пароль: 7–30 символів, латиниця і цифри";
      ...
      <Dialog open={isSuccess} onOpenChange={setIsSuccess}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Ваш обліковий запис успішно створено. Вас будуть перенаправлено на
              сторінку входу.
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
